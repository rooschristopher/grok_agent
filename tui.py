...
class TUIChatApp(App):
    ...
    def on_mount(self) -> None:
        ...
        self.log.write(f'🚀 [bold green]Ready![/] [cyan]{self.target_dir}[/]')
        self.log.write('[dim]/help for cmds | Rich CLI: grok-chat[/]')
    ...
    def handle_command(self, cmd: str) -> None:
        ...
        self.log.write(f'[magenta]Cmd: {cmd}[/]')
        if cmd_lower == '/help':
            self.log.write('''
[bold]Commands:[/]
...
CLI companion: grok-chat
            ...
            self.log.write(f'[yellow]Git:[/] {status}')
        elif cmd_lower == '/subagents':
            subs = self.agent.list_subagents()
            self.log.write(f'[blue]Subs:[/] {subs}')
            ...
            self.log.clear()
            self.log.write('[dim]Cleared[/]')
        elif cmd_lower in ('/quit', '/q', 'quit'):
            self.exit()
        else:
            self.log.write('[red]Unknown. /help[/]')

    def _agent_turn(self) -> None:
        max_steps = 20
        for step in range(1, max_steps + 1):
            self.log.write(f'[dim]🤖 Step {step}/{max_steps}...[/]')
            ...
            if not tool_calls:
                ...
                return
            self.log.write(f'[orange]{len(tool_calls)} tools...[/]')
    ...
    def on_agent_update(self, event: AgentUpdate) -> None:
        if event.is_tool:
            self.log.write(f'[bold yellow]🛠️ Tool:[/] {event.content}')
        else:
            self.log.write(f'[bold blue]🤖 :[/] {event.content}')
