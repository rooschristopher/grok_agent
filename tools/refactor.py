...
def refactor(target_dir: Path, filename: str, client, model: str, apply_fixes: bool = False) -> str:
    ...
    issues_summary = '\\n'.join(
        [f"{issue.get('message', 'Unknown')} [{issue.get('symbol', '')}] at line {issue.get('location', {}).get('line', '?')}" for issue in issues[:10]]
    )
...
"""
    ...
    try:
        ...
        result = {
            ...
            'preview': suggested_code[:200] + '...' if len(suggested_code) > 200 else suggested_code,
