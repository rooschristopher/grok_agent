...
If you cannot determine a fix, use empty fix_actions. Do not add extra fields."""
    ...
    try:
        ...
        return {
            "error": str(e),
            "raw_response": getattr(msg, "content", "") if "msg" in locals() else "",
        }
...
def auto_debug(
        ...
        for action in actions:
            ...
            try:
                if atype == "write_file":
                    ...
                    log.append(
                        f"  Shell RC: {ar.returncode}, out: {ar.stdout.strip() or ar.stderr.strip()}"
                    )
                    applied += 1
    ...
    return {
        "success": False,
        "reason": f"Failed after {max_iters} iterations",
