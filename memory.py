...
class ChromaMemory:
    def __init__(self, persist_directory: str):
        ...
        self.chats_coll = chroma_client.get_or_create_collection(
            name="chats", metadata={"hnsw:space": "cosine"}
        )
    ...
    def add_chat_messages(self, chat_id: str, messages: list[dict[str, Any]]):
        ...
        for i, msg in enumerate(messages):
            ...
            metadatas.append({"chat_id": chat_id, "role": msg["role"], "turn": i})
            ids.append(f"c_{chat_id}_{i}")
        if documents:
            self.chats_coll.add(documents=documents, metadatas=metadatas, ids=ids)
    ...
    def add_agent_experience(
        ...
        self.agent_coll.add(documents=[doc], metadatas=[metadata], ids=[doc_id])

    def query_agent_memory(self, query: str, n_results: int = 5) -> dict:
        return self.agent_coll.query(query_texts=[query], n_results=n_results)
